...
class DdareungiToS3(DdareungiBaseClass):
    def __init__(self, app_name):
        ...
        self.log_mode = 'DEBUG'
        self.checkpoint_location = "hdfs:///path/to/checkpoint"  # HDFS 체크포인트 위치 설정
        self.state_store_location = "hdfs:///path/to/state_store"  # HDFS 상태 저장소 위치 설정
    ...
    def load_state(self):
        ...
        spark = self.create_spark_session()
        try:
            state_df = spark.read.parquet(self.state_store_location)
        except:
            # 상태 정보가 없으면 빈 DataFrame 생성
    ...
    def save_state(self, state_df):
        # HDFS에 상태 정보 저장
        state_df.write.mode("overwrite").parquet(self.state_store_location)
